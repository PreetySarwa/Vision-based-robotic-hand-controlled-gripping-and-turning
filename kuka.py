    ...
    if (self.useInverseKinematics):
      ...
      for action in range(len(motorCommands)):
        ...
        p.setJointMotorControl2(self.kukaUid,
                                ...
                                force=self.maxForce)
  def get_individual_image(self, x, y, z, a):
    look = [x, y, 0.43]
    ...
    img_arr = p.getCameraImage(width=512,
                            height=512,
                            viewMatrix=viewmatrix,
                            projectionMatrix=proj_matrix,
                            renderer=p.ER_BULLET_HARDWARE_OPENGL)
    rgb = img_arr[2]
    np_img_arr = np.reshape(rgb, (512, 512, 4))
    np_img_arr = cv2.cvtColor(np_img_arr,cv2.COLOR_BGR2RGB)
    depth = img_arr[3]
    depth = np.reshape(depth, (512, 512))
    #print(depth.shape)
    ...
    cv2.imwrite(r"depth"+str(a)+".jpeg",depth_image_new)
    cv2.imwrite(r"color"+str(a)+".jpeg",np_img_arr)                             
